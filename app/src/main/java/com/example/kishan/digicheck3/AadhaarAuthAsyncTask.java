...
import com.example.kishan.digicheck3.util
.GsonSerializerUtil;
...
public class AadhaarAuthAsyncTask extends AsyncTask<String, Void, AuthResponse> {
	...
	AuthCaptureRequest authData;
    String aaduid;
	public AadhaarAuthAsyncTask(Context ctx,String aaduid, AuthCaptureRequest authCaptureData) {
	...
	protected void onPreExecute() {
		...
		mDialog.setMessage("Please wait while verifying your details with the server");
		mDialog.show();
	}
	...
	protected void onPostExecute(AuthResponse response) {
        ...
        {
            FetchData fetchData = new FetchData(aaduid,ctx);
            fetchData.execute();
        }
        else {
